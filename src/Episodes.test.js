...
test("Shows episodes", () => {
    const { rerender, queryAllByTestId, getAllByTestId} =
    render(<Episodes episodes={[]} />);
    
    const episodeList = queryAllByTestId(/episode/i);
    expect(episodeList).toHaveLength(0);

    rerender(<Episodes episodes={episodesData}/>)
    const episodeLength = getAllByTestId(/episode/i);
    expect(episodeLength).toHaveLength(3);
})
